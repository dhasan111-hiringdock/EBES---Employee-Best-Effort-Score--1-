...
export default function EditRoleModal({ isOpen, role, onClose, onSuccess }: EditRoleModalProps) {
  ...
  const [assignedRecruiterIds, setAssignedRecruiterIds] = useState<number[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetchWithAuth(`/api/rm/roles/${role.id}`, {
        method: 'PUT',
      ...
      <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="sticky top-0 bg-gradient-to-r from-indigo-600 to-indigo-700 px-8 py-6 flex items-center justify-between text-white z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-2xl font-bold">Edit Role</h3>
              <p className="text-indigo-100 text-sm mt-0.5 font-mono">{role.role_code}</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="p-8 overflow-y-auto max-h-[calc(90vh-180px)]">
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mb-3">A role can be assigned to multiple teams</p>
                ...
                {recruiters.length === 0 ? (
                  ...
                  recruiters.map(recruiter => (
                    ...
                    >
                      ...
                      {assignedRecruiterIds.includes(recruiter.id) && (
                        <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full font-medium">
                          Currently Assigned
                        </span>
                      )}
